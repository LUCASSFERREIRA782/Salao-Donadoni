...
Cliente atual: **DONADONI** (salão de beleza — Vila Luzita, Santo André)
...
Todas as cores vivem em `css/variables.css`, nas variáveis `--color-*`. Alterar ali muda o site inteiro, incluindo o gradiente holográfico (`--gradient-holo`), sem precisar mexer em nenhum outro CSS.
...
O template não depende de nada específico de salão — troque os textos de `config.js` (serviços, sobre, diferenciais) pelo conteúdo do novo nicho (barbearia, clínica, restaurante etc.) e ajuste a paleta em `variables.css` pra combinar com a nova identidade visual.
